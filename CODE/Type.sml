...
struct
  ...
  fun checkStat s vtable ftable resultT =
    ...
    | S100.Throw (e,p) =>
        if checkExp e vtable ftable = Int
        then ()
        else raise Error ("Exception type should be integer",p)
    | S100.TryCatch (s1,sid,s2,p) =>
        let
          val vtable1 = [(getName(sid),Int)]@vtable
        in
          (checkStat s2 vtable1 ftable resultT;
           List.app (fn s => checkStat s vtable ftable resultT) s1)
        end
  ...
  fun checkReturn s =
    ...
    | S100.Throw (i,p) => true
    | S100.TryCatch (s1,id,s2,p) => List.exists checkReturn s1 andalso checkReturn s2
  ...
  fun checkProg fs =
    let
      val ftable = getFuns fs [("unhandled", ([Int],Int)),
                               ("walloc",([Int],Ref Int)),
			       ("balloc",([Int],Ref Char)),
			       ("getint",([],Int)),
